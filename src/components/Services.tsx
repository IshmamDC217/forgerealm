...
export default function Services() {
  ...
  return (
    <section
      id="services"
      className="theme-surface py-24 bg-[linear-gradient(135deg,_#0b0b0e_0%,_#101018_60%,_#6366f1_60%,_#3b82f6_100%)] relative overflow-hidden"
    >
      ...
      <div ref={ref} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
          ...
          {[
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                {service.icon}
                <h3 className="text-lg font-semibold text-[color:var(--fg)]">{service.title}</h3>
              </div>
              <p className="mt-2 text-sm text-[color:var(--fg-muted)]">{service.text}</p>
            </article>
          ))}
        </div>
        ...
        <div className="max-w-2xl mt-16 rounded-lg border border-[color:var(--border)] surface-strong backdrop-blur-2xl p-6 hover:border-blue-400 transition light-panel">
          ...
          <div className="flex items-center gap-4 text-[color:var(--fg-muted)] mt-2">
